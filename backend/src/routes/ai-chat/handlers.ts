...
async function resolveTaskId(db: any, args: any): Promise<string | null> {
  ...
  const hit = rows.find(
    (r: any) => normalizeSearchText(r.title).includes(norm) || r.title.includes(kw),
  )
  return hit?.id ?? null
}
...
async function resolveListId(db: any, args: any): Promise<string | null> {
  ...
  const norm = normalizeSearchText(kw)
  const hit = rows.find(
    (r: any) => normalizeSearchText(r.name).includes(norm) || r.name.includes(kw),
  )
  return hit?.id ?? null
}

async function resolveKbDoc(
  db: any,
  args: any,
  ...
  if (args.docId) {
    const rows = await db
      ...
      .where(eq(schema.kbDocuments.id, String(args.docId)))
      .limit(1)
    return rows[0] ?? null
  }
  const kw = (args.keyword || '').toString().trim()
...
async function handleCreateTask(c: Context<{ Bindings: Env }>, db: any, args: any, ctx: ChatCtx) {
  ...
  if (wantName) {
    const hit =
      ctx.lists.find((l) => l.name === wantName) ||
  ...
  if (!listId) {
    ...
    await db
      ...
      .values({ id, name: wantName || '默认', color: '#2563EB', sortOrder: 0, isSystem: false })
    listId = id
  ...
  const existing = await db
    .select({ s: schema.tasks.sortOrder })
    .from(schema.tasks)
  ...
  const parts = [
    dueDate ? `截止 ${dueDate}` : '',
    bool(args.isImportant) ? '已标重要' : '',
    bool(args.isMyDay) ? '已加入我的一天' : '',
  ].filter(Boolean)
  return {
    observation: `已创建任务「${title}」${createdList}${parts.length ? `（${parts.join('、')}）` : ''}`,
    refresh: true,
  }
}
  ...
  const rows = await db
    ...
    .where(
      includeCompleted
        ...
        : and(eq(schema.tasks.isCompleted, false), isNull(schema.tasks.msTodoDeletedAt)),
    )
...
async function handleCompleteTask(
  ...
  if (!id)
    return {
      observation: `没找到要操作的任务（${args.keyword || args.id || '无关键词'}）`,
      refresh: false,
    }
  const cur = await db
  ...
  await db
    .update(schema.tasks)
    .set({ isCompleted: true, updatedAt: nowBeijing() })
    .where(eq(schema.tasks.id, id))
  try {
    await db
...
async function handleAddNote(c: Context<{ Bindings: Env }>, db: any, args: any, _ctx: ChatCtx) {
  ...
  return {
    observation: `已保存笔记：${title}`,
    refresh: false,
    action: { type: 'navigate', payload: '/notes' },
  }
}
...
async function handleSearchNotes(
  ...
  const matched = rows
    .filter(
      (r: any) =>
        normalizeSearchText(r.title).includes(norm) ||
        normalizeSearchText(r.content).includes(norm),
    )
    .slice(0, 5)
...
async function handleUpdateAiConfig(
  ...
  if (type === 'openai' && !args.baseUrl) {
    return {
      observation: 'OpenAI 类型需要提供 baseUrl（如 https://api.deepseek.com/v1）',
      refresh: false,
    }
  }
  ...
  const existing = await dbx
    ...
    .where(eq(schema.aiConfigs.name, name))
    .limit(1)
...
async function handleCreateTaskList(
  ...
  await db.insert(schema.taskLists).values({
    id,
    name,
    color: args.color ? str(args.color) : '#2563EB',
    sortOrder: maxSort + 1,
    isSystem: false,
  })
...
async function handleUpdateTaskList(
  ...
  if (!listId)
    return {
      observation: `没找到要修改的列表（${args.keyword || args.listId || '无关键词'}）`,
      refresh: false,
    }
  ...
  const cur = await db
    .select({ name: schema.taskLists.name })
    .from(schema.taskLists)
...
async function handleDeleteTaskList(
  ...
  if (!listId)
    return {
      observation: `没找到要删除的列表（${args.keyword || args.listId || '无关键词'}）`,
      refresh: false,
    }
  const cur = await db
    .select({ name: schema.taskLists.name })
...
async function handleCreateSubtask(
  ...
  const taskId = args.taskId
    ? String(args.taskId)
    : args.taskKeyword
      ? await resolveTaskId(db, { keyword: str(args.taskKeyword) })
      : null
  if (!taskId)
    return {
      observation: `没找到要添加子任务的任务（${args.taskKeyword || args.taskId || '无关键词'}）`,
      refresh: false,
    }
  const title = str(args.title)
  ...
  if (!id && (args.taskKeyword || args.title)) {
    ...
    if (taskId) {
      ...
      id =
        subs.find(
          (s: any) =>
            s.title.includes(kw) || normalizeSearchText(s.title).includes(normalizeSearchText(kw)),
        )?.id ?? null
    }
  }
  if (!id)
    return {
      observation: `没找到要操作的子任务（${args.taskKeyword || args.title || args.subtaskId || '无关键词'}）`,
      refresh: false,
    }
  const existing = await db
...
async function handleUpdateNote(c: Context<{ Bindings: Env }>, db: any, args: any, _ctx: ChatCtx) {
  ...
  if (!id && args.keyword) {
    const rows = await db
      .select({ id: schema.imaNotes.id, title: schema.imaNotes.title })
      .from(schema.imaNotes)
      .limit(100)
    ...
    id =
      rows.find(
        (r: any) =>
          normalizeSearchText(r.title).includes(norm) || r.title.includes(str(args.keyword)),
      )?.id ?? null
  }
  if (!id)
    return {
      observation: `没找到要修改的笔记（${args.keyword || args.noteId || '无关键词'}）`,
      refresh: false,
    }
  const patch: any = { updatedAt: nowBeijing() }
...
async function handleDeleteNote(c: Context<{ Bindings: Env }>, db: any, args: any, _ctx: ChatCtx) {
  ...
  if (!id)
    return {
      observation: `没找到要删除的笔记（${args.keyword || args.noteId || '无关键词'}）`,
      refresh: false,
    }
...
async function handleSearchKnowledge(
  ...
  const list = matched
    .map((m: any) => `· ${m.title}：${(m.content || '').slice(0, 120).replace(/\n/g, ' ')}`)
    .join('\n')
  return { observation: `知识库匹配到 ${matched.length} 篇：\n${list}`, refresh: false }
}

async function handleSummarizeKnowledge(
  c: Context<{ Bindings: Env }>,
  db: any,
  args: any,
  _ctx: ChatCtx,
  ...
  if (!doc)
    return {
      observation: `没找到要总结的文档（${args.keyword || args.docId || '无关键词'}）`,
      refresh: false,
    }
  const content = (doc.content || '').slice(0, 8000)
  if (!content.trim())
    return { observation: `《${doc.title}》暂无可用正文，无法总结`, refresh: false }
  try {
    ...
    return {
      observation: `《${doc.title}》开头：${(doc.content || '').slice(0, 200)}`,
      refresh: false,
    }
  }
}
...
async function handleAskKnowledge(
  ...
  if (!doc)
    return {
      observation: `没找到要提问的文档（${args.keyword || args.docId || '无关键词'}）`,
      refresh: false,
    }
  const content = (doc.content || '').slice(0, 6000)
  if (!content.trim())
    return { observation: `《${doc.title}》暂无可用正文，无法问答`, refresh: false }
  try {
