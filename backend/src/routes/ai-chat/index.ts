    ...
    async start(controller) {
      ...
      try {
        await streamChat(c, db, {
          ...
          systemPrompt, role, images, webSearch,
          ctx,
          send,
        })
      } catch (e: any) {
...
async function streamChat(
  ...
  db: any,
  opts: { message: string; sessionId: string; history: { role: string; content: string }[]; ctx: any; send: (o: any) => void; deepThink?: boolean; webSearch?: boolean; systemPrompt?: string; role?: string; images?: string[] }
): Promise<void> {
  const { message, sessionId, history, ctx, send, deepThink, webSearch, systemPrompt, role, images } = opts
  ...
  if (webSearch) hints.push('用户已开启「联网搜索」，请优先使用 web_search 工具获取最新信息再回答。')
      ...
      if (result.toolCalls && result.toolCalls.length) {
        ...
        for (const tc of result.toolCalls) {
          ...
          try {
            ...
            messages.push({ role: 'assistant', content: null, tool_calls: [{ id: callId, type: 'function', function: { name: tc.name, arguments: JSON.stringify(tc.args) } }] })
            messages.push({ role: 'tool', tool_call_id: callId, name: tc.name, content: toolResult.observation })
          } catch (e: any) {
