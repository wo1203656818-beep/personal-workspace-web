...
export function useChatSessions({
  ...
  const removeSession = useCallback(
    async (id: string, currentSessionId: string | null, e: React.MouseEvent) => {
      e.stopPropagation()
      try {
  ...
  const togglePin = useCallback(async (s: ChatSessionPreview) => {
    ...
    try {
      ...
      setSessions((list) =>
        [...list.map((x) => (x.id === s.id ? { ...x, pinned: next } : x))].sort(
  ...
  const saveTags = useCallback(
    async (editingSession: ChatSessionPreview | null, tags: string[]) => {
