    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-80 rounded-2xl border border-white/10 bg-[#1a1a1a] p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-2 flex flex-wrap gap-1.5">
          {['回答尽量简短', '多用列表和表格', '解释得通俗一点', '像朋友一样聊天'].map((s) => (
            <button
              key={s}
              type="button"
              onClick={() => setCustomPrompt(customPrompt.includes(s) ? customPrompt : (customPrompt ? customPrompt + '；' : '') + s)}
        ...
        <div className="mt-3 flex gap-2">
          ...
          <button type="button" onClick={onClose} className="flex-1 rounded-xl bg-primary py-2 text-[12px] font-medium text-primary-foreground hover:bg-primary/90">完成</button>
        </div>
      </div>
    </div>,
    document.body
