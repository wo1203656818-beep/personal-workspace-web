      ...
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2">
            <Settings className="w-5 h-5" /> 订阅源管理
          </SheetTitle>
        ...
        <div className="px-4 space-y-4">
          <Button
            ...
            onClick={() => {
              if (
                confirm(
                  '确定要重置所有资讯源吗？这会删除所有现有源并替换为精选源（约 30 个高质量源）。已抓取的新闻也会被清空。',
                )
              ) {
                onReset()
              }
          ...
          {sources ? (
            Object.entries(filteredSourcesByCategory as Record<string, NewsSource[]>).map(
              ([cat, catSources]) => (
                <div key={cat}>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setNewSource((s) => ({ ...s, category: v }))}
                >
                  <SelectTrigger className="w-full">
