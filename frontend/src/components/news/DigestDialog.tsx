...
import type { NewsDigestItem as DigestItem } from '@/lib/api/news'
      ...
      <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <CalendarDays className="w-5 h-5" /> 历史简报
          </DialogTitle>
            ...
            digests.map((digest) => {
              ...
              return (
                <div key={digest.id} className="border rounded-lg overflow-hidden">
                  <button
                    onClick={() => onToggleDigest(digest.id)}
                      ...
                      <div className="space-y-1.5">
                        {topItems.map((t, i) => (
                          <div key={i} className="flex items-start gap-2 text-sm">
                            <span className="shrink-0 w-5 h-5 rounded-full bg-primary text-primary-foreground text-xs flex items-center justify-center mt-0.5">
                              {i + 1}
                            </span>
                            <div className="min-w-0">
                              {t.url ? (
