...
export const taskListsApi = {
  ...
  create: (data: { name: string; color?: string }) =>
    api.post('tasks/lists', { json: data }).json<TaskList>(),
  update: (id: string, data: { name?: string; color?: string }) =>
    api.put(`tasks/lists/${id}`, { json: data }).json<TaskList>(),
...
export const tasksApi = {
  ...
  create: (data: Partial<Task>) => api.post('tasks', { json: data }).json<Task>(),
  ...
  reorder: (orders: { id: string; sortOrder: number }[]) =>
    api.put('tasks/reorder', { json: { orders } }).json(),
  // 行动承诺系统
...
export const subtasksApi = {
  ...
  create: (taskId: string, title: string, sortOrder?: number) =>
    api.post(`subtasks/${taskId}`, { json: { title, sortOrder } }).json<Subtask>(),
  toggle: (id: string) => api.patch(`subtasks/${id}/toggle`).json<Subtask>(),
  reorder: (orders: { id: string; sortOrder: number }[]) =>
    api.put('subtasks/reorder', { json: { orders } }).json(),
