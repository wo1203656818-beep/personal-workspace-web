...
export const monitorApi = {
  ...
  createTarget: (data: Partial<MonitorTarget>) =>
    api.post('monitor/targets', { json: data }).json<{ ok: boolean; id: string }>(),
  updateTarget: (id: string, data: Partial<MonitorTarget>) =>
    api.put(`monitor/targets/${id}`, { json: data }).json(),
  ...
  runPlatform: (platform: string) =>
    api
      .post('monitor/run-platform', { json: { platform } })
      .json<{ ok: boolean; fetched: number; platform: string; error?: string }>(),
